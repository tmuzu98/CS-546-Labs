...
module.exports = {
  async create(name, location, phoneNumber, website, priceRange, cuisines, overallRating, serviceOptions) {
    if (!name && !location && !phoneNumber && !website && !priceRange && !cuisines && !overallRating && !serviceOptions) throw 'All fields need to have valid values';
    isString(name);
    isString(location);
    ...
    if(typeof serviceOptions.dineIn!="boolean" || typeof serviceOptions.takeOut!="boolean" || typeof serviceOptions.delivery!="boolean"  ) throw "Invalid Entries for service Options";
  ...
  async remove(id) {
    ...
    const restaurantCollection = await restaurants();
    let parsedId= ObjectId(id);
    const deletionInfo = await restaurantCollection.removeOne({ _id: parsedId });
    ...
    return "Restaurant with " +id+" has been successfullt deleted.";
},
async rename(id, newWebsite)
{
    if (!id) throw 'You must provide an id to search for';
    if (!newWebsite) throw 'You must provide a new website';
    //isString(id);
    isString(newWebsite);
    const regex2= /^(http|https):\//;
    var match = regex2.test(newWebsite);
    if(match===false) {
    throw "Enter proper website";
    }
    let p=newWebsite.endsWith(".com");
    if(p==false)
    {
    throw "Enter proper website";
    }
    ...
    const restaurantCollection = await restaurants();
    /* if(restaurantCollection.find( { website: { $eq: newWebsite } } ))
    {
    throw "Same Website";
    }  */
    let parsedId= ObjectId(id)
    const renameInfo=restaurantCollection.updateOne(
        { _id: parsedId },
        { $set:
           {
             website: newWebsite
     ...
     if (renameInfo.modifiedCount === 0) {
        throw 'could not update website successfully or Id doees not exist';
      }
