...
function () {
  ...
  _proto.getNormalizedMap = function getNormalizedMap(options) {
    ...
    if (!map) return null;
    var routeSchema = new normalizr.schema.Entity('routes');
    var namespaceSchema = new normalizr.schema.Entity('namespaces', {
      routes: [routeSchema]
    });
    namespaceSchema.define({
      namespaces: [namespaceSchema]
    });
    return normalizr.normalize(map, {
      namespaces: [namespaceSchema],
      routes: [routeSchema]
    });
  }
...
RouteConfig.fromNormalizedMap = function (normalizedMap, options) {
  ...
  var normalizableMap = normalizr.denormalize(normalizedMap.result, {
    namespaces: [namespaceSchema],
    routes: [routeSchema]
